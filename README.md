...
/universe.1-research project <설명>
    ...
    → /universe.5-execute          (Wave 1회 실행)
    → /universe.5-execute --auto   (전체 자동 실행)
        ↕
...
`00-index.md`의 Wave 구성을 기반으로 task를 병렬 실행하는 단계입니다. `--auto` 옵션으로 모든 task가 완료될 때까지 자동 반복할 수 있습니다. Phase가 완료될 때마다 `phase-reviewer` (devil's advocate)가 실제 코드를 읽고 설계 문서와 대조 검증합니다.
...
**실행 전략 (모드 공통):**
- `00-index.md`의 "병렬 Wave" 섹션에서 현재 실행할 Wave를 결정합니다
- Wave 내 task 1개: 직접 실행 (team 없음)
- Wave 내 task 2~4개: agent team으로 병렬 실행
- Wave 내 task 5개 이상: 우선순위 상위 4개를 선택하여 병렬 실행
...
**실행 흐름:**
1. PROGRESS.md + 00-index.md 읽기 → 현재 Wave 파악
2. 팀 리드가 PROGRESS.md 상태를 `진행중`으로 일괄 변경
3. worker들이 병렬로 task 구현 및 테스트 검증
4. worker 완료 후 팀 리드가 PROGRESS.md를 `완료`/`차단됨`으로 일괄 업데이트
5. Phase 완료 시 → `phase-reviewer` 검증 실행
6. `--auto`면 → 다음 Wave로 자동 반복

**PROGRESS.md 관리:** worker는 구현과 결과 보고만 수행하고, PROGRESS.md는 팀 리드가 일괄 관리합니다. 병렬 실행 시 동시 편집 충돌을 방지합니다.
...
| `/universe.5-execute` | 1~4 worker + phase-reviewer | 1~4 worker + phase-reviewer | Wave 병렬→Phase 검증 (`--auto` 반복) |
...
**Q: `--auto` 옵션은 언제 사용하나요?**
A: task가 많고 중간에 개입할 필요가 없을 때 사용합니다. Wave 단위로 병렬 실행을 자동 반복하며, Phase 검증에서 심각도 상 문제가 발견되거나 모든 task가 완료되면 자동으로 중단됩니다.
